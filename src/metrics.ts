...
class MetricsInstruments {
  ...
  initialize(serviceName: string): void {
    if (!this.instruments) {
      this.instruments = this.createInstruments(serviceName);
    }
  }
  ...
  private createInstruments(serviceName: string): Instruments {
    const meter = metrics.getMeter(serviceName ?? 'Unknown');
...
export const initializeMetrics = (serviceName: string): void => {
